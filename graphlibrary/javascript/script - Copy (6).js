;(function(){
	...
	function TopicDiv(){
		...
		//this.topicElement.style.background = 'red';
		...
		this.createHistogram = function(dataStack){
			...
			for(var i=0;i<dataValue.length;i++){
				...
				if(i<=11){
					...
					ctx.fillRect(currX + 110,30*(j+1),15,15);
					ctx.fillText(dataName[i],currX+135 , 30*(j+1)+5);
					ctx.fillText(dataValue[i],currX+135, 30*(j+1)+20);
					j++;
				}
			}

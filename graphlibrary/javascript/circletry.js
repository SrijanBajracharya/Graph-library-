...
//debugger;
				...
				var startX = 0;
				var startY = dataSet[0]+2;
				var finalX = sections*xScale.toFixed(2);
				context.beginPath();
				context.moveTo(startX,startY);
				for(var i=1;i<sections;i++){
					var endX = i*xScale.toFixed(2);
					var endY = dataSet[i];
					displayLineGraphSlowly(context,startX,startY,endX,endY,i,finalX,finalY,sections,xScale,dataSet);
				};
				
			};

			
				var displayLineGraphSlowly = function(context,startX,startY,endX,endY,i,finalX,finalY,sections,xScale,dataSet){
					var count = 0;
					var slope = 0;
					var distance = 0;
					var smallDistance = 0.00005;
					var timeInterval = setInterval(function(){
						//debugger;
						/*console.log('haha');
						//context.lineTo(endX,endY);
						var incx = startX + (endX-startX)*smallDistance;
						var incy = startY + (endY-startY)*smallDistance;
						startX = endX;
							startY = endY;
						context.lineTo(incx,incy);
						smallDistance+=.000001;																
						context.stroke();
						if(finalY < incy){
							console.log('yes');
						...
						//context.lineTo(endX,endY);
						context.lineTo(startX + (endX-startX)*smallDistance,startY + (endY-startY)*smallDistance);
						count++;
						context.stroke();
						startX = endX;
						startY = endY+2;
						if(slope === '-Infinity' || distance === 'NaN'){
							clearInterval(timeInterval);
						}
		...
		this.createPiChart = function(dataStack){
            ...
            var colors = ["#990099","#109618","#ff9900","#dc3912","#3366cc","#004411","#7E3817", "#2c3e50", "#7f8c8d", "#2980b9", "#1abc9c", "#f39c12", "#307D7E","#5136f6", "#e2f5b4","#fc081f", "#E0FFFF", "#9edd08","#fb1ef8","#fdfbb4", "#faf406","#fbd4b7", "#f2700f","#f8bdb4", "#ea2507","#e2bcbd", "#9e2126","#bbddb3", "#1d8e04"];
		...
		this.createHistogram = function(dataStack){
			...
			for(var i=maxVal;i>=0;i-=1){
				if(Math.ceil(stepSize*i<=maxVal)){
					...
					ctx.fillText(Math.ceil(stepSize*i),currX-40,canvas.height-i*stepSize*3);
					ctx.stroke();
				}
			};
			...
			for(var i=0;i<dataValue.length;i++){
				ctx.fillStyle = colors[i];
				ctx.font = "8pt Arial";
				ctx.fillRect(currX + 50,30*(i+1),15,15);
				ctx.fillText(dataName[i],currX + 75 , 30*(i+1)+5);
				ctx.fillText(dataValue[i],currX + 75, 30*(i+1)+20);
			}
	...
	function LineGraph(){
		...
		this.lineGraph.style.background = 'url(images/linegraph.png)';
	...
	function Upload(){
		...
		this.upload.setAttribute('value','Upload file');
		this.upload.style.float = 'right';
		this.upload.style.paddingLeft = '10px';
		this.upload.style.background = 'url(images/choosefile1.png)';
		this.upload.style.marginLeft = '30px';
		this.upload.style.height = '20px';

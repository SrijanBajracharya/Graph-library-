;(function(){
	...
	function Canvas(){
		...
		this.createLineGraph = function(dataStack){
			...
			var drawLineGraph = function(){
				...
				displayLineGraphSlowly(context,columnSize,rowSize,maxVal,minVal,sections);
				...
				function plotData(dataSet) {
					context.beginPath();
					context.strokeStyle="#109618";
					context.moveTo(0, dataSet[0]+2);
					context.arc(0,dataSet[0]+2,1,6*Math.PI,7*Math.PI); 
					for (var i=1;i<sections;i++) {
						context.lineTo(i * xScale, dataSet[i]+2);
						context.arc(i*xScale,dataSet[i]+2,2,6*Math.PI,7*Math.PI);
					}
					context.stroke();
			};
			drawLineGraph();
			function displayLineGraphSlowly(context,columnSize,rowSize,maxVal,minVal,sections){
				context.fillStyle = '#0099ff';
				context.font = '12px Open sans';
	...
	function Histogram(){
		...
		//this.histogram.innerHTML = 'Histogram';
		//this.histogram.style.color = 'AliceBlue ';

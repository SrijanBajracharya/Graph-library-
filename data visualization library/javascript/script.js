;(function(){
	...
	function TopicDiv(){
		...
		this.topicElement.innerHTML = 'Data Visualization Library';
	...
	function Canvas(){
		...
		this.createLineGraph = function(dataStack){
			...
			function plotData(dataSet,sections,finalY) {
				console.log(sections);
				context.beginPath();
				if(sections>10 && sections<13){
					context.moveTo(0, dataSet[0]+2);
					context.fillRect(0, dataSet[0]+2,6,3);
				}
				else if(sections === 10){
					context.moveTo(1*sections, dataSet[0]+2);
					context.fillRect(1*sections, dataSet[0]+2,6,3);
				}

				else if(sections>=8 && sections<10){
					context.moveTo(2*sections,dataSet[0]+2);
					context.fillRect(2*sections,dataSet[0]+2,6,3);
				}
				else if(sections>6 && sections<8){
					context.moveTo(3*sections,dataSet[0]+2);
					context.fillRect(3*sections,dataSet[0]+2,6,3);
				}
				else if(sections === 6){
					context.moveTo(8*sections,dataSet[0]+2);
					context.fillRect(8*sections,dataSet[0]+2,6,3);
				}

				else if(sections<6 && sections>=3){
					context.moveTo(11*sections,dataSet[0]+2);
					context.fillRect(11*sections,dataSet[0]+2,6,3);
				}
				...
				for(var i=1;i<sections;i++){
					xCoord.push(Math.ceil(i*xScale));
				};
				var counter = 0;
				var timeInterval = setInterval(function(){
					if(sections>10 && sections<13){
						...
						context.fillRect(point,dataSet[counter]+1,6,3);
						
					}

					else if(sections === 10){
						var point = xCoord[counter++];
						context.lineTo(point+sections, dataSet[counter]+2);
						context.fillRect(point+sections,dataSet[counter]+1,6,3);
					}

					else if(sections>=8 && sections<10){
						var point = xCoord[counter++];
						context.lineTo(point+2*sections, dataSet[counter]+2);
						context.fillRect(point+2*sections,dataSet[counter]+1,6,3);
					}

					else if(sections>6 && sections<8){
						var point = xCoord[counter++];
						context.lineTo(point+5*sections, dataSet[counter]+2);
						context.fillRect(point+5*sections,dataSet[counter]+1,6,3);
					}
					else if(sections===6){
						var point = xCoord[counter++];
						context.lineTo(point+8*sections, dataSet[counter]+2);
						context.fillRect(point+8*sections,dataSet[counter]+1,6,3);
					}

					else if(sections<6 && sections>=3){
						var point = xCoord[counter++];
						context.lineTo(point+13*sections, dataSet[counter]+2);
						context.fillRect(point+13*sections,dataSet[counter]+1,6,3);
					}
            ...
            for(var i in dataValue) { 
                ...
                for(var i=0;i<dataValue.length;i++){
                    ...
                    if(j<7){ 
	                    ...
	                    ctx.fillRect(570,(i+1)*50,15,15);
	                    ctx.fillText(dataName[i],525,45+(i*50));
	                    ctx.fillText(degree[i].toFixed(2) + '\xB0',525,60+(i*50));
	                    ctx.fillText(percentage[i].toFixed(2) + '%',525,74+(i*50));
	                }

	                else if(j>=7){
	                	ctx.font = '9pt Open sans';
	                	ctx.fillRect(60,(k+1)*50,15,15);
	                	ctx.fillText(dataName[i],20,45+(k*50));
	                	ctx.fillText(degree[i].toFixed(2) + '\xB0',20,60+(k*50));
	                    ctx.fillText(percentage[i].toFixed(2) + '%',20,74+(k*50));
	                	k++;
	                }

	                else if(j>13){
	                	ctx.font = '9pt Open sans';
	                	ctx.fillRect(40,(l+1)*50,15,15);
	                	ctx.fillText(dataName[i],0,45+(l*50));
	                	ctx.fillText(degree[i].toFixed(2) + '\xB0',10,60+(l*50));
	                    ctx.fillText(percentage[i].toFixed(2) + '%',10,74+(l*50));
	                	l++;
	                }
		...
		this.createHistogram = function(dataStack){
			...
			for(var i=0;i<dataValue.length;i++){
				...
				ctx.font = "12px Open sans";
				if(i<9){
					ctx.fillRect(currX + 50,40*(i+1),15,15);
					ctx.fillText(dataName[i],currX + 75 , 40*(i+1)+5);
					ctx.fillText(dataValue[i],currX + 75, 40*(i+1)+20);
				}
				else if(i>=9){
					ctx.fillRect(currX + 110,40*(j+1),15,15);
					ctx.fillText(dataName[i],currX+140 , 40*(j+1)+5);
					ctx.fillText(dataValue[i],currX+140, 40*(j+1)+20);
					j++;
				}
			}
		...
		this.createBarDiagram = function(dataStack){
        	...
        	function init() {
            	...
            	for (i = 0; i < dataValue.length; i++) {
                	calcY(dataValue[i]);
                	ctx.fillText(dataName[i], xScalar * (i + 1), y - margin-10);
                	ctx.fillStyle = colors[i];
            	}
	...
	function DataDiv () {
		...
		this.createForm = function(){
			...
			this.btnDeleteAll = document.createElement('input');
			this.btnDeleteAll.setAttribute('type', 'button');
			this.btnDeleteAll.setAttribute('value','Delete All');
			this.btnDeleteAll.style.color = 'white';
			this.btnDeleteAll.setAttribute('id','btnDeleteAll');
			this.btnDeleteAll.style.borderCollapse = 'collapse';
			this.btnDeleteAll.style.background = 'url(images/bbtn.png)';
			this.btnDeleteAll.style.border = '0px';
			this.btnDeleteAll.style.height = '20px';
			this.btnDeleteAll.style.marginLeft = '10px';
			this.btnDeleteAll.style.width = '69px';
			this.btnDeleteAll.style.cursor = 'pointer';
			...
			this.element.appendChild(this.btnDeleteAll);
		};		
	};
	...
	function DataHandler(){
	    ...
	    this.updateDelete = function(){
	    	console.log('delete all');
	    	for(var i=0;i<that.dataStack.length;i++){
	    		table.removeChild(document.getElementById('row-'+ that.dataStack[i].id));	
	    	};
	    	for(var i=0;i<that.dataStack.length;i++){
	    		that.dataStack.splice(i,that.dataStack.length);
	    	};
	    	counter=0
	    };
	...
	function Chart (mainDiv_) {
		...
		if (mainDiv_ && mainDiv_.className) {
			className = mainDiv_.className;
		}
